...
public class SeekArea : Area
{
    ...
    private Array<Character> enemiesInArea = [];
    private Array<NPC> alliesInArea = [];
    private Array<float> attackTimer = [];
    ...
    public override void _Process(float delta)
    {
        ...
        var npcState = npc.GetState();

        switch (npcState)
        {
            case SetStateEnum.Talk:
                return;
    ...
    private void UpdateSearchState()
    {
        ...
        if (tempEnemy < enemiesInArea.Count)
        {
            ...
            {
                if (!enemiesInArea.Contains(tempVictim))
                {
                    enemiesInArea.Add(tempVictim);
                }
                ...
                if (attackTimer[tempEnemy] >= ATTACK_TIME)
                {
                    ...
                    npc.SetState(
                        npc.Weapons is null or { HasWeapon: true }
                            ? SetStateEnum.Attack
                            : SetStateEnum.Hiding
                    );
                }
    ...
    public void _on_seekArea_body_entered(Node body)
    {
        ...
        if (npc.relation == Relation.Neitral
            ...
            || character is NPC { MayChangeState: false })
        {
            return;
        }
